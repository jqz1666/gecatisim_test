...
extern "C" {
// Read-only bridge for transferring the CPU-owned voxelized phantom to CUDA.
DLLEXPORT int get_phantom_info_vox_for_cuda(int MaterialIndex, void **volume, int *dims, float *offsets,
  float *voxelSize, unsigned char **xyMask, float **muTable, int *materialCount,
  int *eBinCount, int *volumeDataType)
  {
  if(volume == NULL || dims == NULL || offsets == NULL || voxelSize == NULL || xyMask == NULL ||
    muTable == NULL || materialCount == NULL || eBinCount == NULL || volumeDataType == NULL ||
    materials.materialCount < 1)
    return -1;

  if(phantom.volume == NULL || phantom.dims == NULL || phantom.xy_mask == NULL ||
    materials.muTable == NULL || MaterialIndex < 1 || MaterialIndex > materials.materialCount)
    return -2;

  int index = MaterialIndex - 1;
  *volume = phantom.volume[index];
  dims[0] = phantom.dims[index][0];
  dims[1] = phantom.dims[index][1];
  dims[2] = phantom.dims[index][2];
  offsets[0] = phantom.xoff[index];
  offsets[1] = phantom.yoff[index];
  offsets[2] = phantom.zoff[index];
  voxelSize[0] = phantom.dxy[index];
  voxelSize[1] = phantom.dxy[index];
  voxelSize[2] = phantom.dz[index];
  *xyMask = phantom.xy_mask[index];
  *muTable = materials.muTable;
  *materialCount = materials.materialCount;
  *eBinCount = materials.eBinCount;
  *volumeDataType = useUInt16 ? 1 : 0;
  return 0;
  }
}
