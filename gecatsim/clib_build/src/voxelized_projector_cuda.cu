...
struct CudaPhantom_cache
{
    int valid;
    int device_id;
    unsigned long long phantom_signature;
    int n_materials;
    int n_energy;
...
static int ensure_phantom_cache(
    int device_id,
    const void *const *volume_data,
    const int *dims,
    const float *volume_offsets_xyz,
    const float *voxel_size,
    const float *mu,
    const unsigned char *const *xy_mask,
    unsigned long long phantom_signature,
    int volume_data_type,
    int n_materials,
    int n_energy)
{
    if (g_cuda_phantom.valid &&
        ...
        g_cuda_phantom.phantom_signature == phantom_signature &&
        g_cuda_phantom.volume_data_type == volume_data_type &&
    ...
    if (volume_data_type != 0 && volume_data_type != 1)
    {
        return CUDA_PROJECTOR_BAD_ARGUMENT;
    }

    int64_t *volume_offsets = (int64_t *)malloc((size_t)(n_materials + 1) * sizeof(int64_t));
    int64_t *xy_mask_offsets = (int64_t *)malloc((size_t)(n_materials + 1) * sizeof(int64_t));
    if (!volume_offsets || !xy_mask_offsets)
    {
        free(volume_offsets);
        free(xy_mask_offsets);
    ...
    {
        if (!volume_data[material] || !xy_mask[material] ||
            dims[material * 3 + 0] <= 0 || dims[material * 3 + 1] <= 0 || dims[material * 3 + 2] <= 0)
        {
            free(volume_offsets);
            free(xy_mask_offsets);
            return CUDA_PROJECTOR_BAD_ARGUMENT;
        }
        volume_offsets[material + 1] = volume_offsets[material] +
            (int64_t)dims[material * 3 + 0] * (int64_t)dims[material * 3 + 1] * (int64_t)dims[material * 3 + 2];
        xy_mask_offsets[material + 1] = xy_mask_offsets[material] +
            (int64_t)dims[material * 3 + 0] * (int64_t)dims[material * 3 + 1] * 2;
    }

    int64_t volume_data_count = volume_offsets[n_materials];
    int64_t xy_mask_count = xy_mask_offsets[n_materials];

    size_t voxel_bytes = volume_data_type == 1 ? sizeof(unsigned short) : sizeof(float);
    size_t volume_bytes = (size_t)volume_data_count * voxel_bytes;
    ...
    for (int material = 0; material < n_materials && err == cudaSuccess; ++material)
    {
        size_t material_bytes = (size_t)(volume_offsets[material + 1] - volume_offsets[material]) * voxel_bytes;
        err = cudaMemcpy((unsigned char *)g_cuda_phantom.d_volume_data + (size_t)volume_offsets[material] * voxel_bytes,
            volume_data[material],
            material_bytes, cudaMemcpyHostToDevice);
    }
    if (err == cudaSuccess)
    ...
    for (int material = 0; material < n_materials && err == cudaSuccess; ++material)
    {
        size_t mask_bytes = (size_t)(xy_mask_offsets[material + 1] - xy_mask_offsets[material]) * sizeof(unsigned char);
        err = cudaMemcpy(g_cuda_phantom.d_xy_mask + xy_mask_offsets[material], xy_mask[material],
    ...
    float *trans_out)
{
    ...
    for (int src_id = 0; src_id < n_sources; ++src_id)
    {
        ...
        for (int mat_id = 0; mat_id < n_materials; ++mat_id)
        {
            ...
            float path_value = paths[path_idx];
            if (volume_data_type == 1)
            {
                path_value *= 0.0001f;
            }
            p_value += path_value * mu[energy * n_materials + mat_id];
        }
...
extern "C"
{
        ...
        int device_id)
    {
        ...
        if (!volume_data || !dims || !volume_offsets_xyz || !voxel_size || !mu ||
            !xy_mask || !src_samples || !src_weights || !det_cell_coords ||
            !det_mod_types || !det_mod_coords || !det_uvecs || !det_vvecs || !trans_out)
        {
            return CUDA_PROJECTOR_BAD_ARGUMENT;
        }
        if (det_cells_per_mod <= 0 || n_modules <= 0 || n_materials <= 0 || n_energy <= 0 ||
            n_pixels <= 0 || n_sources <= 0)
        {
            return CUDA_PROJECTOR_BAD_ARGUMENT;
        }
...
#undef ALLOC_PTR
        ...
        if (err == cudaSuccess)
        {
            if (volume_data_type == 1)
            {
                dd3_row_kernel<unsigned short><<<row_blocks, threads>>>(
                    (const unsigned short *)g_cuda_phantom.d_volume_data, g_cuda_phantom.d_volume_offsets, g_cuda_phantom.d_dims, g_cuda_phantom.d_volume_offsets_xyz, g_cuda_phantom.d_voxel_size,
            ...
            {
                dd3_row_kernel<float><<<row_blocks, threads>>>(
                    (const float *)g_cuda_phantom.d_volume_data, g_cuda_phantom.d_volume_offsets, g_cuda_phantom.d_dims, g_cuda_phantom.d_volume_offsets_xyz, g_cuda_phantom.d_voxel_size,
                    g_cuda_phantom.d_xy_mask, g_cuda_phantom.d_xy_mask_offsets, d_src_samples, d_detZ, d_detX, d_scales,
                    d_x0, d_y0, d_z0, d_vertical, nrdetcols, nrdetrows, n_materials, n_sources,
                    max_rows, d_views);
            }
        ...
        if (err == cudaSuccess)
        {
            combine_trans_kernel<<<trans_blocks, threads>>>(
                d_paths, g_cuda_phantom.d_mu, d_src_weights, n_materials, n_energy, n_pixels, n_sources,
                volume_data_type, d_trans);
            err = cudaGetLastError();
            if (err == cudaSuccess)
                err = cudaDeviceSynchronize();
            if (err == cudaSuccess)
