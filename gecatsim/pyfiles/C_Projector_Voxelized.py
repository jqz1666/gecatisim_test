...
def C_Projector_Voxelized(cfg, viewId, subViewId):
    backend = getattr(cfg.physics, "projectorBackend", "cpu").lower()
    if backend in ("gpu", "auto"):
        try:
            from gecatsim.pyfiles.C_Projector_Voxelized_CUDA import C_Projector_Voxelized_CUDA, is_cuda_projector_available
            if is_cuda_projector_available(cfg):
                return C_Projector_Voxelized_CUDA(cfg, viewId, subViewId)
            if not getattr(cfg.sim, "_cudaProjectorWarningPrinted", False):
                print("***WARNING: Native CUDA voxelized projector is unavailable; falling back to CPU projector.")
                cfg.sim._cudaProjectorWarningPrinted = True
        except Exception as err:
            if not getattr(cfg.sim, "_cudaProjectorWarningPrinted", False):
                print(f"***WARNING: Native CUDA voxelized projector failed to initialize ({err}); falling back to CPU projector.")
                cfg.sim._cudaProjectorWarningPrinted = True
