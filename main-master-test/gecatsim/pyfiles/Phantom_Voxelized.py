...
def Phantom_Voxelized(cfg):
    ...
    Mus = set_material(cfg, materialList)
    cfg.phantom.gpuVoxelData = {
        "materials": materialList,
        "mus": np.ascontiguousarray(Mus, dtype=np.single),
        "volumes": []
    }
    ...
    for i in range(numberOfMaterials):
        ...
        cfg.phantom.gpuVoxelData["volumes"].append({
            "data": np.ascontiguousarray(volumeData, dtype=np.single),
            "dims": np.ascontiguousarray(dims, dtype=np.int32),
            "offsets": np.ascontiguousarray(offsets, dtype=np.single),
            "voxelsize": np.ascontiguousarray(voxelsize, dtype=np.single),
            "xyMask": np.ascontiguousarray(xyMask, dtype=np.uint8),
        })
